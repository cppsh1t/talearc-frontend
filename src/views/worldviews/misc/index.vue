  ...
  <div class="h-full flex flex-col bg-gray-50">
    <div class="p-4 bg-white shadow-sm flex items-center justify-between mb-2">
      <div class="flex items-center gap-2">
        ...
        <h2 class="text-lg font-bold">杂项管理</h2>
      </div>
    ...
    <div class="flex-1 overflow-auto">
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 p-4">
        <MiscCard v-for="item in list" :key="item.id" :data="item" @edit="handleEdit" @delete="handleDelete" />
      </div>
    </div>
    ...
    <MiscDialog v-model:visible="dialogVisible" :type="dialogType" :data="currentMisc" :world-view-id="worldViewId"
      @success="fetchData" />
  </div>
</template>
...
const queryParams = ref<MiscQuery>({
  page: 1,
  size: 12,
...
function handleAdd() {
  ...
  currentMisc.value = undefined
  dialogVisible.value = true
}

function handleEdit(item: Misc) {
  dialogType.value = 'edit'
  currentMisc.value = item
  dialogVisible.value = true
}

async function handleDelete(item: Misc) {
  try {
    await ElMessageBox.confirm('确定要删除该杂项吗？', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })

    const res = await withDisplay(deleteMisc(item.id), '删除成功')
